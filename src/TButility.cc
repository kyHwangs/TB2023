...
void TButility::loading(const std::string& path) {
  ...
  for ( int i = 0; i < mapChain->GetEntries(); i++ ) {
    ...
    if ( cases == 13 || cases == 14 ) {
      TBdetector det;
      if ( cases == 13 ) det = TBdetector(TBdetector::detid::MCPPMT);
      if ( cases == 14 ) det = TBdetector(TBdetector::detid::SiPM);
    ...
    TBdetector det; 
    if ( cases == 10 ) det = TBdetector(TBdetector::detid::SFHS);
    if ( cases == 11 ) det = TBdetector(TBdetector::detid::LEGO);
    if ( cases == 12 ) det = TBdetector(TBdetector::detid::MCPPMT_gen);
