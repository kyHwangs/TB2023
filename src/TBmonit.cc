...
: fConfig(TBconfig(fConfig_, fRunNum_)), fRunNum(fRunNum_), fMaxEvent(-1) {
...
void TBmonit<T>::Loop() {
  ...
  // TBread<TBwaveform> readerWave = TBread<TBwaveform>(fRunNum, fMaxEvent, "/Users/swkim/DRC/2022_TB_at_CERN/data_sample", enginePlot.getMIDtoLoad());
...
void TBmonit<T>::LoopFast() {
  ...
  TBread<TBfastmode> readerFast = TBread<TBfastmode>(fRunNum, fMaxEvent, "/Users/khwang/scratch/TB2023July/sample", enginePlot.getMIDtoLoad());
