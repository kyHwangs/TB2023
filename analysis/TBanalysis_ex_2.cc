...
int main(int argc, char** argv) {
    ...
    // 2 possible ways to load external pedestal
    ...
    // TButility* util = new TButility(); // Or by using TButility
    // util->loadped("/pnfs/knu.ac.kr/data/cms/store/user/sungwon/2023_DRC_TB_analysis/ped/Run_" + std::to_string(runNum) +"_pedestal.root");
    ...
    std::vector<float> C_counter_1_pedCorrectedWave;
    std::vector<float> C_counter_2_pedCorrectedWave;
    ...
    for (int iEvt = 0; iEvt < totalEntry; iEvt++) {
        ...
        // Load ped directly from pedestal map histogram
        float MCP_sum_C_ped = pedMap->GetBinContent( MCP_sum_C_cid.mid(), MCP_sum_C_cid.channel() );
        float MCP_sum_S_ped = pedMap->GetBinContent( MCP_sum_S_cid.mid(), MCP_sum_S_cid.channel() );
        float C_counter_1_ped = pedMap->GetBinContent( C_counter_1_cid.mid(), C_counter_1_cid.channel() );
        float C_counter_2_ped = pedMap->GetBinContent( C_counter_2_cid.mid(), C_counter_2_cid.channel() );

        // Or one can also use
        // float MCP_sum_C_ped = util->retrievePed(MCP_sum_C_cid);
        // float MCP_sum_S_ped = util->retrievePed(MCP_sum_S_cid);

        MCP_sum_C_pedCorrectedWave.clear();
        MCP_sum_S_pedCorrectedWave.clear();
        C_counter_1_pedCorrectedWave.clear();
        C_counter_2_pedCorrectedWave.clear();
        for (int bin = 0; bin < 1000; bin++) {
            ...
            C_counter_1_pedCorrectedWave.push_back(C_counter_1_ped - C_counter_1_waveform[waveformBin]);
            C_counter_2_pedCorrectedWave.push_back(C_counter_2_ped - C_counter_2_waveform[waveformBin]);
        }
        ...
        float C_counter_1_PeakADC = *std::max_element(C_counter_1_pedCorrectedWave.begin() + 100, C_counter_1_pedCorrectedWave.begin() + 700); // Calculate PeakADC of Ceren. counter 1 with [100, 700) bin
        float C_counter_2_PeakADC = *std::max_element(C_counter_2_pedCorrectedWave.begin() + 100, C_counter_2_pedCorrectedWave.begin() + 700); // Calculate PeakADC of Ceren. counter 2 with [100, 700) bin
    ...
    // Save the outputs
    std::string outFile = "./Sample_analysis_Run_" + std::to_string(runNum) + "_ex_2.root";
